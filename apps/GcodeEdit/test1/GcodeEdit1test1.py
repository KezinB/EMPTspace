...
class GCodeViewer(QWidget):
    ...
    def open_gcode(self):
        fname, _ = QFileDialog.getOpenFileName(self, "Open G-code", "", "G-code Files (*.gcode *.txt)")
        if fname:
    ...
    def populate_layers(self, gcode):
        ...
        for idx, line in enumerate(lines):
            ...
            if re.match(r';\s*LAYER[:\s]', line, re.IGNORECASE) or \
               re.match(r';\s*LAYER_CHANGE', line, re.IGNORECASE) or \
